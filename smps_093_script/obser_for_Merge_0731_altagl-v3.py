...
def read_p3b_smps():
    ...
    if date == '20140731':
        ...
        # # trackpdtime = pd.to_datetime(np.asarray(p3b_smps_time), format='%H:%M:%S').dt.time
        # # trackpdtime=datetime.timedelta(seconds=p3b_smps_time)
        # trackpdtime=pd.to_timedelta(np.asarray(p3b_smps_time), unit='s')
    ...
    plt.pcolormesh(trackpdtime, p3b_diam_list, p3b_data_smps.T, norm=colors.LogNorm(vmin=10, vmax=110000),cmap='RdBu_r')
...
def merge_obs_method():
    ...
    # plt.savefig("p3b_for_qual.jpg", dpi=1500)
...
def aerosol_3_10nm():
    ...
    indexed = df_3_2.set_index(df_3_2['time'], append=True)
    df_2 = indexed.resample('1T', on='time').mean()
    # df_2['N_3nm'].plot(color='black')
    # df_2['N_10nm'].plot()
    df_2['N_3~10nm'].plot(color='black')
    plt.grid(linestyle='-')
    plt.ylabel('N\n (# cm$^{-3}$)')
