...
def read_nav(day, month, campaign, label_on_plot):
    ...
    plt.text(-105.00, 40.05, '*Ground Mobile', fontsize=22, color='red') #I think BAO tower is needed
    ax.coastlines(resolution='50m', linewidth=2)
    ax.set_title("C-130 aircraft track", fontsize=18)
    ...
    norm = BoundaryNorm([500, 1e3, 1.5E3, 2e3, 2.5e3, 3e3, 3.5e3, 4e3, 5e3], cmap.N, clip=False)
    # The flight track
    ...
    plt.savefig("0731C_130_flighttrackd_nonBAO.jpg", dpi=1500)
...
def read_nav_segmentation(day, month, campaign, label_on_plot):
    ...
    ax = plt.subplot(1, 1, 1, projection=ccrs.PlateCarree())
    plt.text(-104.99, 39.74, '*Denver', fontsize=22, color='yellow')
    plt.text(-104.7, 40.2, '*Ground Mobile', fontsize=24, color='yellow')
    plt.text(-105.00, 40.03, '*BAO Tower', fontsize=22, color='yellow')
    ...
    if label_on_plot == 1:
        for itime in range(0, len(hours) - 3600):  # loop for # 19019 times.
            if minutes[itime] == 0 and seconds[itime] == 0:
                ...
                if ((hour >= 12) and (hour <= 20) for hour in hours):
                    ...
                    plt.figure(figsize=(12, 12))
                    ax = plt.subplot(1, 1, 1, projection=ccrs.PlateCarree())
                    plt.text(-104.99, 39.74, '*Denver', fontsize=24, color='yellow')
                    plt.text(-104.7, 40.2, '*Ground', fontsize=24, color='yellow')
                    plt.text(-105.48, 39.38, '*Colorado', fontsize=24, color='yellow')
        ...
        for itime in range(0, len(hours)):  # loop for # 19019 times.
            if minutes[itime] == 0 and seconds[itime] == 0:
                ...
                if ((hour >= 12) and (hour <= 20) for hour in hours):
                    ...
                    cmap.set_over('k')
                    # Altitude color scale
                    norm = BoundaryNorm([500, 1e3, 1.5E3, 2e3, 2.5e3, 3e3, 3.5e3, 4e3, 4.5e3], cmap.N, clip=False)
