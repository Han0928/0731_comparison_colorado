...
def read_flight_data(date):
    ...
    c5h8_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-TOGA_C130_20140728_R2.ict'
...
if date=='20140728':
    timeindexes_glm=['015','018','021','024']  #0726+24h+18h=0727_18pm UTC
    timeindexes_lam=['012','018','024']  #0726+24h+12h=0727_12pm
    lonmin=-112
...
if 'H2SO4' in varlist:
    norm_array_h2so4=[0.0001,0.02,0.05,0.1,0.2,0.3,0.5,0.8,1.0,1.5,3.0,5.0]
    plot_comparison(df,df['H2SO4'],df['model_H2SO4'],df['glm_H2SO4'],(conc_dict['H2SO4'])[1,3,:,:],(conc_dict_glm['H2SO4'])[1,1,:,:],
                    'H2SO4 (pptv)', 0.0001,5.0,0.0001,5.0,False,norm_array_h2so4,'130_h2so4_'+date+'ion_npf_cs093.png', date)
...
if 'NO' in varlist:
    norm_array_no = [5,50,100,300,700,1500,3000]
    plot_comparison(df,df['NO'],df['model_NO'],df['glm_NO'],(conc_dict['NO'])[1,3,:,:],(conc_dict_glm['NO'])[1,1,:,:],
                    'NO (pptv)', 5,3000,20,3000,True,norm_array_no,'130_no_'+date+'ion_npf_cs093.png', date)
