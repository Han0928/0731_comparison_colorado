...
from obser_for_Merge_0729_altagl import merge_obs_method
...
def do_4d_interpolation(cubelist,flight_coords, is_lam,is_dump):
    ...
    epoch = datetime.datetime(2014, 7, 29)
...
if date=='20140728':
    timeindexes_glm=['036','039','042']  #0728+24h+12h=0729_12pm
    timeindexes_lam=['036','042'] #0728+24h+12h=0729_12pm
    lonmin=-112
...
if make_dataset==1:
    ...
    df.to_csv('interpolated_0729_lighenting_'+date+'.csv')
else:
    df = pd.read_csv('interpolated_0729_lighenting_'+date+'.csv',index_col=0, parse_dates=True)
    # print df.head()
    conc_dict = load_model(timeindexes_lam, path_model,varlist,date,1,0)
