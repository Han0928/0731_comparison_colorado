...
from obser_for_Merge_0806_C130 import merge_obs_method
...
def plot_comparison(df,obs_col,lam_col,glm_col,lam2d,glm2d,ylabel,var_min,var_max,map_min,map_max,is_log,norm_array,saving_name,date,plot_obs=1):
    ...
    plt.legend(h2+h3, l2+l3, bbox_to_anchor=(1.0, 1.2), ncol=4, fontsize=10)
...
def load_model(timeindexes,path,varlist,date,is_lam,is_dump):
    ...
    if 'N_All'in varlist or 'N_Nuc'in varlist or 'N_Ait_In' in varlist or 'N_Ait' in varlist or 'N_Acu' in varlist or 'N_Cor' in varlist \
        ...
        # for CCN contribtion, newly added
        outputcubedict["N_100"] = n100_num_conc
        outputcubedict["nuc_CCN"] = nuc_CCN
...
def read_flight_data(date):
    flight_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-NCAR-LRT-NAV_C130_20140806_R4.ict'
    ...
    h2so4_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-OH-H2SO4_C130_20140806_R0.ict'
    ...
    nh3_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-NH3_C130_20140806_R0.ict'
    nh3_time, nh3, dummy, dummy = read_flight_track(nh3_path, 37, 3, 3, 3)
    nh3track = interp_flight_data(tracktime, nh3_time, nh3, -99)  # ppbv!

    so2_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-GTCIMS_C130_20140806_R2.ict'
    so2_time, so2, dummy, dummy = read_flight_track(so2_path, 37, 4, 4, 4)
    so2track = interp_flight_data(tracktime, so2_time, so2, -99)  #ppt

    no_o3_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-NONO2O3_C130_20140806_R0.ict'
    ...
    date = '20140806'
    unix_epoch_of_6August2014_0001 = 1407283200
    if date == '20140806':
        trackpdtime = pd.to_datetime(np.asarray(tracktime) + unix_epoch_of_6August2014_0001, unit='s')
    ...
    d= {'time':pd.Series(trackpdtime), 'latitude':pd.Series(np.asarray(lat)),
        ...
        'NO':pd.Series(notrack),'O3':pd.Series(o3track), 'NH3':pd.Series(nh3track),
        'T':pd.Series(temptrack),'P':pd.Series(pres2track),'SUMNC2':pd.Series(obe_sumnc)} 
    df3 = pd.DataFrame(d)
...
date= '20140806'
varlist = ['NO','NO2','O3','CO','NH3','P','T','SO2','nuc_CCN','N_100',
            'OH', 'H2SO4','SUMNC2']
# make_dataset=0
is_dump=0
if is_dump:
...
path_model='/jet/home/ding0928/cylc-run/u-ct706/share/cycle/20140805T0000Z/Regn1/resn_1/RA2M/um/'
path_glm = '/jet/home/ding0928/cylc-run/u-ct706/share/cycle/20140805T0000Z/glm/um/'
...
if date=='20140806':
    timeindexes_glm=['036','039','042','045']  #0805+36h=0806_12pm
    timeindexes_lam=['036','042'] #0805+36h=0806_12pm
    lonmin=-112
...
if make_dataset==1:
    ...
    df.to_csv('interpolated_0806_c130_'+date+'.csv')
else:
    df = pd.read_csv('interpolated_0806_c130_'+date+'.csv',index_col=0, parse_dates=True)
    print(df.head())
    conc_dict = load_model(timeindexes_lam, path_model,varlist,date,1,0)
...
if 'OH' in varlist:
    ...
    plot_vertical(df,conc_dict['OH'].coord('level_height'),df['OH'],df['model_OH'],df['glm_OH'],1.0, 'OH (pptv)','130_oh'+date+'ion_npf_cs093.png')

if 'T' in varlist:
    norm_array_temp=[270,280,285,290,295]
    ...
    if is_dump:
        plot_comparison(df,df['T'],df['model_T'],df['glm_T'],(conc_dict['T'])[1,10,:,:],(conc_dict_glm['T'])[1,:,:],
                        'T (K)', 280,300,280,300,False,norm_array_temp,'130_T_'+date+'ion_npf_cs093.png', date)
    else:
        plot_comparison(df,df['T'],df['model_T'],df['glm_T'],(conc_dict['T'])[1,10,:,:],(conc_dict_glm['T'])[1,10,:,:],
                        'T (K)', 280,300,280,300,False,norm_array_temp,'130_T_'+date+'ion_npf_cs093.png', date)
    plot_vertical(df,conc_dict['T'].coord('level_height'),df['T'],df['model_T'],df['glm_T'],300, 'T (K)','130_T'+date+'ion_npf_cs093.png',1,280)
...
if 'H2SO4' in varlist:
    ...
    plot_comparison(df,df['H2SO4'],df['model_H2SO4'],df['glm_H2SO4'],(conc_dict['H2SO4'])[1,10,:,:],(conc_dict_glm['H2SO4'])[1,10,:,:],
                    'H2SO4 (pptv)', 0.05,1.5,0.05,1.5,False,norm_array_h2so4,'130_h2so4_'+date+'ion_npf_cs093.png', date)
    plot_vertical(df,conc_dict['H2SO4'].coord('level_height'),df['H2SO4'],df['model_H2SO4'],df['glm_H2SO4'],1.0, 'H2SO4 (pptv)','130_h2so4'+date+'ion_npf_cs093.png')
if 'NH3' in varlist:
    norm_array_nh3=[1,5,10,15,20,25,30,50,100]
    ...
    plot_vertical(df,conc_dict['NH3'].coord('level_height'),df['NH3'],df['model_NH3'],df['glm_NH3'],8, 'NH3 (ppbv)','130_nh3'+date+'ion_npf_cs093.png')
...
if 'SUMNC2' in varlist:
    norm_array_sumnc2 = [0,0.00001,0.0001,0.001,0.005,0.008,0.01,0.02]
    plot_comparison(df,df['SUMNC2'],df['model_SUMNC2'],df['glm_SUMNC2'],(conc_dict['SUMNC2'])[1,10,:,:],(conc_dict_glm['SUMNC2'])[1,10,:,:],
                    'Cond sink (s-1)', 0.001,0.02,0.001,0.02,False,norm_array_sumnc2,'130_sumnc2_'+date+'ion_npf_cs093.png', date)
