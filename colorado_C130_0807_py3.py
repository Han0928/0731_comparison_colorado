...
from obser_for_Merge_0807_C130 import merge_obs_method
...
def get_vertical_profiles(model_height_coord,interp_altitudes,interp_result):
    totals_in_altitude_bins=np.zeros(len(model_height_coord.points)-2)
    numbers_in_altitude_bins=np.zeros(len(model_height_coord.points)-2)
    totals_squared_in_altitude_bins=np.zeros(len(model_height_coord.points)-2)
    model_altitude_bounds = model_height_coord.bounds
    ibin=0
    altitude_bounds_to_use = [[0,0.5]]
    i=0
    altitude_points_to_use=[0.25]
    for height_interval in model_altitude_bounds[3:]:
        altitude_bounds_to_use.append(height_interval)
        altitude_points_to_use.append(model_height_coord.points[i+3])
        i=i+1
...
def plot_vertical(df,model_height_coord,obs,lamvar,glmvar,xmax,xlabel,saving_name, plot_obs=1, x_min=0):
    ...
    plt.yticks([1.5,3.0,4.0,5.0,6.0], [1.5,3.0,4.0,5.0,6.0])
...
def read_flight_data(date):
    flight_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-NCAR-LRT-NAV_C130_20140807_R4.ict'
    ...
    h2so4_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-OH-H2SO4_C130_20140807_R0.ict'
    ...
    nh3_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-NH3_C130_20140807_R0.ict'
    nh3_time, nh3, dummy, dummy = read_flight_track(nh3_path, 37, 3, 3, 3)
    nh3track = interp_flight_data(tracktime, nh3_time, nh3, -99)  # ppbv!

    so2_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-GTCIMS_C130_20140807_R2.ict'
    so2_time, so2, dummy, dummy = read_flight_track(so2_path, 37, 4, 4, 4)
    so2track = interp_flight_data(tracktime, so2_time, so2, -99)  #ppt

    no_o3_path = '/jet/home/ding0928/Colorado/Colorado/FRAPPE-NONO2O3_C130_20140807_R0.ict'
    ...
    date = '20140807'
    unix_epoch_of_7August2014_0001 = 1407369600
    if date == '20140807':
        trackpdtime = pd.to_datetime(np.asarray(tracktime) + unix_epoch_of_7August2014_0001, unit='s')
...
date= '20140807'
varlist = ['NO','NO2','O3','CO','NH3','P','T','SO2',
            'OH', 'H2SO4','SUMNC2']
# make_dataset=0
is_dump=0
if is_dump:
...
path_model='/jet/home/ding0928/cylc-run/u-cs093/share/cycle/20140807T0000Z/Regn1/resn_1/RA2M/um/'
path_glm = '/jet/home/ding0928/cylc-run/u-cs093/share/cycle/20140807T0000Z/glm/um/'
...
if date=='20140807':
    timeindexes_glm=['015','018','021','024']  #0807+12h=0807_12pm
    timeindexes_lam=['012','018','024'] #0805+36h=0806_12pm
    lonmin=-112
...
if make_dataset==1:
    ...
    df.to_csv('interpolated_0807_c130_'+date+'.csv')
else:
    df = pd.read_csv('interpolated_0807_c130_'+date+'.csv',index_col=0, parse_dates=True)
    print(df.head())
    conc_dict = load_model(timeindexes_lam, path_model,varlist,date,1,0)
