...
from obser_for_Merge_p3b import merge_obs_method
...
def plot_comparison(df,obs_col,lam_col,glm_col,lam2d,glm2d,ylabel,var_min,var_max,map_min,map_max,is_log,norm_array,saving_name,date,plot_obs=1):
    ...
    plt.savefig('/jet/home/ding0928/Colorado/Colorado/p-3b/fig_11.9'+saving_name)
...
def read_flight_data(date):
    ...
    d= {'time':pd.Series(trackpdtime), 'latitude':pd.Series(np.asarray(lat)),'longitude':pd.Series(np.asarray(lon)),
        'altitude':pd.Series(np.asarray(alt_track)),'N_All':pd.Series(np.asarray(n3track)),'N_10':pd.Series(np.asarray(n10track)),
        'N_Nuc':pd.Series(nnuctrack),'C5H8':pd.Series(c5h8track),'C10H16':pd.Series(c10h16track),
...
path_model='/jet/home/ding0928/cylc-run/u-co218/share/cycle/20140729T0000Z/Regn1/resn_1/RA2M/um/'
path_glm = '/jet/home/ding0928/cylc-run/u-co218/share/cycle/20140729T0000Z/glm/um/'
...
if 'NH3' in varlist:
    norm_array_nh3=[0.1,0.2,0.3,0.5,0.7,0.9,1.2,1.5,2,3,5,7,10]
...
if 'N_Nuc' in varlist:
    ...
    plot_comparison(df,df['N_Nuc'],df['model_N_Nuc'],df['glm_N_Nuc'],(conc_dict['N_Nuc'])[1,3,:,:],(conc_dict_glm['N_Nuc'])[1,1,:,:],
                    'N_Nuc(3-10nm) (#cm-3)', 100,1e4,100,1e4,False,norm_array_oh,'korus_nnuc_'+date+'_bt116.png', date)
    plot_vertical(df,conc_dict['N_Nuc'].coord('level_height'),df['N_Nuc'],df['model_N_Nuc'],df['glm_N_Nuc'],1e4, 'N_Nuc (#cm-3)','korus_N_Nuc_vp_'+date+'_bt116.png')
