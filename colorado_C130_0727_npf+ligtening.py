...
def plot_vertical(df,model_height_coord,obs,lamvar,glmvar,xmax,xlabel,saving_name, plot_obs=1, x_min=0):
    ...
    plt.yticks([1.5,2.0,3.0], [1.5,2.0,3.0])
...
def read_flight_data(date):
    ...
    d= {'time':pd.Series(trackpdtime), 'latitude':pd.Series(np.asarray(lat)),
        ...
        'C5H8':pd.Series(c5h8track),'NO':pd.Series(notrack),
...
if 'T' in varlist:
    ...
    if is_dump:
        ...
        plot_comparison(df,df['T'],df['model_T'],df['glm_T'],(conc_dict['T'])[1,10,:,:],(conc_dict_glm['T'])[1,10,:,:],
                        'T (K)', 280,300,280,300,False,norm_array_temp,'130_T_'+date+'ion_npf_cs093.png', date)
    plot_vertical(df,conc_dict['T'].coord('level_height'),df['T'],df['model_T'],df['glm_T'],310, 'T (K)','130_T'+date+'ion_npf_cs093.png',1,270)

if 'H2SO4' in varlist:
    norm_array_h2so4=[0.0001,0.02,0.05,0.1,0.2,0.3,0.5,0.8,1.0,1.5,3.0]
    plot_comparison(df,df['H2SO4'],df['model_H2SO4'],df['glm_H2SO4'],(conc_dict['H2SO4'])[1,10,:,:],(conc_dict_glm['H2SO4'])[1,10,:,:],
                    'H2SO4 (pptv)', 0.0001,3.0,0.0001,3.0,True,norm_array_h2so4,'130_h2so4_'+date+'ion_npf_cs093.png', date)
...
if 'NH3' in varlist:
    ...
    plot_comparison(df,df['NH3'],df['model_NH3'],df['glm_NH3'],(conc_dict['NH3'])[1,10,:,:],(conc_dict_glm['NH3'])[1,10,:,:],
                    'NH3 (ppbv)', 0.2,20,0.2,20,True,norm_array_nh3,'130_nh3_'+date+'ion_npf_cs093.png', date)
    plot_vertical(df,conc_dict['NH3'].coord('level_height'),df['NH3'],df['model_NH3'],df['glm_NH3'],20, 'NH3 (pptv)','130_nh3'+date+'ion_npf_cs093.png')
...
if 'NO' in varlist:
    norm_array_no = [5,50,100,300,700,1500]
    plot_comparison(df,df['NO'],df['model_NO'],df['glm_NO'],(conc_dict['NO'])[1,10,:,:],(conc_dict_glm['NO'])[1,10,:,:],
                    'NO (pptv)', 5,1500,20,1500,True,norm_array_no,'130_no_'+date+'ion_npf_cs093.png', date)
...
if 'SUMNC2' in varlist:
    ...
    plot_comparison(df,df['SUMNC2'],df['model_SUMNC2'],df['glm_SUMNC2'],(conc_dict['SUMNC2'])[1,10,:,:],(conc_dict_glm['SUMNC2'])[1,10,:,:],
                    'Cond sink (s-1)', 0.001,0.05,0.001,0.05,True,norm_array_sumnc2,'130_sumnc2_'+date+'ion_npf_cs093.png', date)
plt.show()
