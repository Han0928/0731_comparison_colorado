...
def plot_comparison(df,col_label,lam2d,glm2d,ylabel,var_min,var_max,map_min,map_max,is_log,norm_array,saving_name,date,two_models,df2,plot_obs=1):
    ...
    plt.tight_layout()
    plt.savefig('/jet/home/ding0928/box_model/data/overall_code/rerun_NH3_emission/neutralization/ground_obs_npf/'+saving_name)
...
def plot_sizedistribution(df_smps,saving_name):
    ...
    all_sizes = 1e-3*np.array(sizebins_p3b+las_sizes_p3b) #nm->um
    plt.figure()
...
def read_flight_data(date):
    flight_path='/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-reveal_p3b_20140729_r0.ict'
    ...
    c5h8_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-PTRTOF-NMHCs_P3B_20140729_R0.ict'
    ...
    no_o3_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-NOXYO3_P3B_20140729_R0.ict'
    ...
    co_p3b_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-DACOM_P3B_20140729_R1_L1.ict'
    ...
    n_nuc_p3b_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-CNC_P3B_20140729_R0.ict'
    ...
    n100path='/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-LAS_P3B_20140729_R0.ict'
    n100time,n100_all,dummy,dummy = read_flight_track(n100path,69,1,1,1) ## LAS is all -9999 for 0727, thus discard!!
    print('n100',len(n100time),len(n100_all),len(tracktime))
    n100track_all = interp_flight_data(tracktime,n100time,n100_all, -99)

    smps_path_p3b='/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-SMPS_P3B_20140729_R0.ict'
    ...
    if date == '20140729':
        trackpdtime = pd.to_datetime(np.asarray(tracktime) + unix_epoch_of_29July2014_0001, unit='s')

    smpspdtime = pd.to_datetime(np.asarray(twod_smps['Mid_UTC'])+unix_epoch_of_29July2014_0001,unit='s')
...
path_model='/jet/home/ding0928/cylc-run/u-ct706/share/cycle/20140728T0000Z/Regn1/resn_1/RA2M/um/'
path_glm = '/jet/home/ding0928/cylc-run/u-ct706/share/cycle/20140728T0000Z/glm/um/'
...
if date=='20140729':
    timeindexes_glm=['036','039','042']  #0722+24h+12h=0723_12pm
    timeindexes_lam=['036','042'] #0722+24h+12h=0723_12pm
    lonmin=-112
    lonmax=-97        #
    latmin=35
    latmax=46
...
if make_dataset==1:
    ...
    df.to_csv(path+'0729_ct076_dataset_'+date+'.csv')
    df_smps.to_csv(path+'0729_ct076_smps_dataset_'+date+'.csv')
    
    sys.exit()
else:
    df = pd.read_csv(path+'0729_ct076_dataset_'+date+'.csv',index_col=0, parse_dates=True)
    if two_models==1:
        df2 = pd.read_csv(path+'0729_ct076_dataset_'+date+'.csv',index_col=0, parse_dates=True)
    else:
        df2=None
    # print df.head()
    if date=='20160510':
...
df_smps = pd.read_csv(path+'0729_ct076_smps_dataset_'+date+'.csv',index_col=0, parse_dates=True)
print(df_smps)
plt.figure()
df_smps['nSMPS_stdPT'].plot(label='nSMPS_obs')
df_smps['model_N'] = df_smps['model_AitN']+df_smps['model_AInsN']+df_smps['model_AccN']+df_smps['model_NucN']+df_smps['model_CorN']
df_smps['model_N'].plot(label='n_model')
plt.legend()
df_smps_1 = df_smps.between_time(start_time='14:40', end_time='15:00').mean()
plot_sizedistribution(df_smps_1,'ct076-729L_08:40-09:0.png')
df_smps_2 = df_smps.between_time(start_time='16:00', end_time='16:20').mean()
plot_sizedistribution(df_smps_2,'ct076-729L_10:00-10:20.png')
df_smps_3 = df_smps.between_time(start_time='17:45', end_time='18:00').mean()
plot_sizedistribution(df_smps_3,'ct076-729L_11:45-12:00.png')
df_smps_4 = df_smps.between_time(start_time='17:30', end_time='17:40').mean()
plot_sizedistribution(df_smps_4,'ct076-729H_11:30-11:40.png')
# df_smps_5 = df_smps.between_time(start_time='22:50', end_time='23:00').mean()
# plot_sizedistribution(df_smps_5,'093-731L_16:50-17:00.png')
# df_smps_6 = df_smps.between_time(start_time='19:30', end_time='19:40').mean()
# plot_sizedistribution(df_smps_6,'093-731H_13:30-13:40.png')
