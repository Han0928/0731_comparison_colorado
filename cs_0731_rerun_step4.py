...
def merge_obs_method():
    # the 0802 parameter loading from NAV.ict
    temp = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/FRAPPE-NCAR-LRT'
                      '-NAV_C130_20140731_R4.ict', delimiter=',',
                      skiprows=132, usecols=29)  # inC  (17401, )
    pres = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/FRAPPE-NCAR-LRT'
                      '-NAV_C130_20140731_R4.ict', delimiter=',',
                      skiprows=132, usecols=62)  # in hpa  (17401, )

    time_NAV = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/FRAPPE-NCAR-LRT'
                          '-NAV_C130_20140731_R4.ict',
                          delimiter=',',
                          skiprows=132, usecols=0)
    print('time_NAV.shape', time_NAV.shape)  # ('time_NAV.shape', (17401,))
    print('temp.shape', temp.shape)  # ('temp.shape', (17401,))
    print('pres.shape', pres.shape)  # ('pres.shape', (17401,))
    # in 0731 where the resolution is tricky
    # temp = np.concatenate((temp, [0]))  # to go from 16380 to 16381 since data_100 is 16381
    ...
    file_100 = Dataset(
        '/jet/home/ding0928/Colorado/EOLdata/dataaw2NKX/RF05Z.20140731.195400_011100.PNI.nc')  # (16381, 31)
    ...
    # check the CONCP_RPI to verify the number concentration
    # print('concp_rpi', file_100['CONCP_RPI'])
    concp_rpi_100 = (file_100.variables['CONCP_RPI'][:])
    # plt.plot(uhsas_time, concp_rpi_100, '-')
    # plt.legend(['CONCP_RPI(PCAS) # cm-3'])
    # plt.ylim(10, 5000)
    # plt.yscale('symlog', linthreshy=0.01)
    # plt.title('0731 from uhsas')
    # plt.savefig('/jet/home/ding0928/box_model/data/overall_code/rerun_NH3_emission/neutralization/ground_obs_npf'
    #             '/0731_comparison/concp_0731.png')
    # plt.figure()
    # # extra fig to zoom in the details
    # plt.plot(uhsas_time, concp_rpi_100, '-')
    # plt.legend(['CONCP_RPI(PCAS) # cm-3'])
    # plt.ylim(10, 5000)
    # plt.yscale('symlog', linthreshy=0.01)
    # plt.title('0731 from uhsas for zoom in')
    # plt.savefig('/jet/home/ding0928/box_model/data/overall_code/rerun_NH3_emission/neutralization/ground_obs_npf'
    #             '/0731_comparison/concp_0731_zoom_in.png')
    # plt.figure()
    ...
    print('term2_shape', term2.shape)  # ('term2_shape', (19021,)
    # mask out the <0 for data_100
    data_100 = ma.masked_where(data_100 < 0, data_100)
    uhsas_time2 = uhsas_time[~data_100[:, 0].mask]
    data_100_2 = data_100[~data_100[:, 0].mask]

    print('uhsas_time2.shape', uhsas_time2.shape)  # ('uhsas_time2.shape', (19021,))

    # print('time_NAV_2shape', time_NAV_2.shape) #since here we don't need NAV_2 from line 24

    gas_track_data2 = []
    interp_gas = interp1d(time_NAV, term2, kind='nearest',
                          fill_value='extrapolate')
    gas_track = interp_gas(np.asarray(uhsas_time2))
    gas_track_data2.append(gas_track)
    # print('gas_track_data2', gas_track_data2)
    print('gas_track_data2_shape', np.asarray(gas_track_data2).shape)  # ('gas_track_data2_shape', (1, 19021))

    data_100_new3 = []
    # data_100_new = ma.mask_rowcols(data_100, axis=0)
    data_100_new2 = data_100_2[~data_100_2.mask.any(axis=1)]
    # print(data_100_new2)
    print('data_100_new2.shape', data_100_new2.shape)  # ('data_100_new2.shape', (19000, 31))
    ...
    time_NAV_2 = (time_NAV[~(data_100_2.mask.any(axis=1))])  # ('time_NAV_2.shape', (17397,))
    temp_2 = (temp[~(data_100_2.mask.any(axis=1))])
    # temp_2 = temp[~data_100_new.mask]  # masked t
    pres_2 = (pres[~(data_100_2.mask.any(axis=1))])

    term1 = (term1[~(data_100_2.mask.any(axis=1))])  # 80-83 is a replace for the loop
    term2 = (term2[~(data_100_2.mask.any(axis=1))])
    print('temp_2.shape', temp_2.shape)
    print('pres_2.shape', pres_2.shape)
    print('term1.shape', term1.shape)
    print('term2.shape', term2.shape)

    print('time_NAV_2.shape', time_NAV_2.shape)  # (17397,))
    ...
    c = range(20, 34)  # 20-33,does not include 34
    ...
    # print('data_smps', data_smps)  # (202*15), 2d
    ...
    for i in range(14):
        ...
        # print('gas_track_data_shape', np.asarray(gas_track_data).shape)
    ...
    # plt.plot(np.divide(time_NAV_2, 3600) - 7, (sumnc_smps + sumnc))
    # plt.legend(['cs_0731'])
    # plt.ylim(0, 0.04)
    # plt.yscale('symlog', linthreshy=0.01)
    # plt.title('0731 for CS')
    # plt.savefig('/jet/home/ding0928/box_model/data/overall_code/rerun_NH3_emission/neutralization/ground_obs_npf'
    #             '/0731_comparison/cs_0731.png')
    # plt.figure()
    ...
    # plt.yscale('symlog', linthreshy=0.01)
    # plt.ylim(0, 0.06)
    # plt.title('0731 for CS_zoom in ')
    # plt.savefig('/jet/home/ding0928/box_model/data/overall_code/rerun_NH3_emission/neutralization/ground_obs_npf'
    #             '/0731_comparison/cs_0731_zoom_in.png')
    # plt.figure()

    return sumnc_smps + sumnc, time_NAV_2


merge_obs_method()
# plt.plot(time_NAV, sumnc)
#
# sumnc = sumnc + sumnc_smps  # this is the total CS=CS(acc+coars)+CS(ait)
# print('sumnc', sumnc)  # this the total cs we would compare with model
