...
def merge_obs_method():
    ...
    temp = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/FRAPPE-NCAR-LRT-NAV_C130_20140731_R4.ict', delimiter=',',
                      skiprows=133, usecols=29)  # inC  (19020, )
    ...
    print('temp.shape', temp.shape)
    print('pres.shape', pres.shape)
    data_100 = (file_100.variables['CS200_RPI'][:, 0, :])  # ('data_100.shape', (16381, 31)
    ...
    gas_track_data2 = []
    interp_gas = interp1d(time_NAV_2, term2, kind='nearest',
                          fill_value='extrapolate')  # smps_time(202,1)? data_smps(202,15) doesn't match?
    gas_track = interp_gas(np.asarray(uhsas_time))  # method borrowed from long script
    ...
    time_NAV_2 = (time_NAV_2[~(data_100.mask.any(axis=1))])  # replace of 64
    temp_2 = (temp[~(data_100.mask.any(axis=1))])
    # temp_2 = temp[~data_100_new.mask]  # masked t
    pres_2 = (pres[~(data_100.mask.any(axis=1))])

    term1 = (term1[~(data_100.mask.any(axis=1))])  # 80-83 is a replace for the loop
    ...
    for i in range(14):
        ...
        data_smps = np.divide(term_3, term_2)  # based on line 48 it should be divide?
    ...
    for i in range(14):
        ...
        nc_smps.append(np.multiply(data_smps[i], cc_smps[i]))
        # nc[3] = acc_num_conc * cc[3]
        # nc[4] = cor_num_conc * cc[4]
    ...
    print('sumnc_smps.shape', sumnc_smps.shape)  # ('sumnc_smps.shape', (19000,))
    # print('sumnc_smps.shape', sumnc_smps.shape)
    print('nc_smps.shape', np.array(nc_smps).shape)  # ('nc_smps.shape', (14, 19000))
    print('kn_smps.shape', np.array(kn_smps).shape)  # ('kn_smps.shape', (14, 19000))
    print('cc_smps.shape', np.array(cc_smps).shape)  # ('cc_smps.shape', (14, 19000))
    print('fkn_smps.shape', np.array(fkn_smps).shape)  # ('fkn_smps.shape', (14, 19000))
    ...
    # plt.plot(np.divide(np.concatenate(time_NAV, [0]), 3600), sumnc[:, 0])
