...
def read_p3b_smps():
    p3b_smps_time = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-SMPS_P3B_20140721_R0.ict',
                               delimiter=',', skiprows=72, usecols=0)  # UTC
    p3b_smps_time = p3b_smps_time - 21600

    date = '20140721'
    unix_epoch_of_21July2014_0001 = 1405900800
    if date == '20140721':
        trackpdtime = pd.to_datetime(np.asarray(p3b_smps_time) + unix_epoch_of_21July2014_0001, unit='s')

    c = range(6, 35)
    p3b_data_smps = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-SMPS_P3B_20140721_R0.ict',
    ...
    plt.ylim(10, 300)
    plt.yscale('log')
    xmin = '2014-07-21 07:30:00'
    xmax = '2014-07-21 13:00:00'
    plt.xlim(xmin, xmax)
...
def merge_obs_method():
    time_NAV = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-reveal_p3b_20140721_r0.ict',
                          delimiter=',',
                          skiprows=73, usecols=0)
    temp = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-reveal_p3b_20140721_r0.ict', delimiter=',',
                      skiprows=73, usecols=18)  # inC
    pres = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-reveal_p3b_20140721_r0.ict', delimiter=',',
                      skiprows=73, usecols=24)  # in hpa

    # load >100nm LAS data for acc+coar
    c = range(5, 31)  # 112nm=6th colomn; 5012nm=32th colomn
    data_las = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-LAS_P3B_20140721_R0.ict',
    ...
    las_time = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE'
                          '-LAS_P3B_20140721_R0.ict', delimiter=',', skiprows=69, usecols=0)
    ...
    gas_track_data2 = []
    for i in range(26):
        data_las_new = data_las[:, i]  # DATA_las~1s
        data_las_new2 = data_las_new[~data_las_new.mask]
        las_time2 = las_time[~data_las_new.mask]

        interp_gas = interp1d(las_time2, data_las_new2, kind='nearest', fill_value='extrapolate')
        gas_track = interp_gas(np.asarray(time_NAV))  # ~1s
    ...
    rr = 8.314  # (J mol^-1 K^-1)=zbolz*avc
    pi = 3.1415927
    ...
    dmol = 4.5e-10  # molecular diameter~0.45nm
    difvol = 51.96
    # sinkarr = 0.0
    ...
    data_smps = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-SMPS_P3B_20140721_R0.ict',
                           delimiter=',', skiprows=72, usecols=c)  # from col 18-33
    data_smps = ma.masked_where(data_smps < 0, data_smps)

    smps_time = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-SMPS_P3B_20140721_R0.ict',
    ...
    for i in range(20):
        ...
        gas_track = interp_gas(np.asarray(time_NAV))  # from 60s to 1s
        gas_track_data.append(gas_track)

        term_3 = np.multiply(gas_track_data, term_1)
    ...
    cut_off_size_1 = [10.0, 11.2, 12.6, 14.1, 15.8, 17.8, 20, 22.4, 25.1, 28.2, 31.6, 35.5, 39.8, 44.7, 50.1, 56.2,
                      63.1, 70.8, 79.4,
                      89.1, 100]  # 21bin, mid-point of size bin of SMPS
    cut_off_size_1 = np.multiply(cut_off_size_1, 1e-9)  # nm to m
    ...
    alt_p3b = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-reveal_p3b_20140721_r0.ict',
                         delimiter=',',
    ...
    unix_epoch_of_21July2014_0001 = 1405900800
    if date == '20140721':
        trackpdtime = pd.to_datetime(np.asarray(time_NAV) + unix_epoch_of_21July2014_0001, unit='s')
...
def aerosol_3_10nm():
    p3b_cNgtnm_smps_time = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE'
                                      '-CNC_P3B_20140721_R0.ict', delimiter=',', skiprows=47, usecols=0)
    ...
    p3b_data_CNgt3 = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE'
                                '-CNC_P3B_20140721_R0.ict', delimiter=',', skiprows=47, usecols=1)
    ...
    p3b_data_CNgt10 = np.loadtxt('/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE'
                                 '-CNC_P3B_20140721_R0.ict', delimiter=',', skiprows=47, usecols=2)
    ...
    unix_epoch_of_21July2014_0001 = 1405900800
    if date == '20140721':
        trackpdtime = pd.to_datetime(np.asarray(p3b_cNgtnm_smps_time) + unix_epoch_of_21July2014_0001, unit='s')
...
def read_flight_data(date):
    alt_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-reveal_p3b_20140721_r0.ict'
    ...
    # alt_time = ma.masked_where(np.asarray(lat) < -90, np.array(alt_time))
    # # print 'filter',len(tracktime[~tracktime.mask]),tracktime[~tracktime.mask]
    # alt_time2 = ma.masked_where(np.asarray(lon) < -180, np.array(alt_time))
    # # print 'filter',len(tracktime[~tracktime.mask])
    # alt_time = (alt_time2[~alt_time.mask])
    # print('alt_time.shape', np.array(alt_time).shape)
    #
    #
    # if len(alt_time.shape) > 1:
    #     alt_time = alt_time[0]
    #     # print 'tracktime.shape',tracktime.shape,np.asarray(lon).shape,np.asarray(lat).shape
    # lon = np.array(lon)[~alt_time2.mask]
    # lat = np.array(lat)[~alt_time2.mask]
    # alt = np.array(alt)[~alt_time2.mask]
    # print('alt_time', alt_time)
    # print('lat', lat)

    flight_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-CNC_P3B_20140721_R0.ict'
    ...
    unix_epoch_of_21July2014_0001 = 1405900800
    if date == '20140721':
        trackpdtime = pd.to_datetime(np.asarray(alt_time) + unix_epoch_of_21July2014_0001, unit='s')
...
def read_groundmobile_smps():
    ground_smps_time = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/DISCOVERAQ'
                                  '-LARGE_GROUND-MOBILE_20140721_R0.ict', delimiter=',', skiprows=109,
                                  usecols=0)  # Seconds after midnight MDT
    c1 = range(31, 41)
    ground_eeps = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/DISCOVERAQ'
                             '-LARGE_GROUND-MOBILE_20140721_R0.ict', delimiter=',', skiprows=109, usecols=c1)

    ground_eeps = ma.masked_where(ground_eeps < 0, ground_eeps)
    c2 = range(44, 55)
    ground_smps = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/DISCOVERAQ'
                             '-LARGE_GROUND-MOBILE_20140721_R0.ict', delimiter=',', skiprows=109, usecols=c2)

    ground_smps = ma.masked_where(ground_smps < 0, ground_smps)

    # eeps:6~150nm smps:10-300nm
    eeps_diam_list = np.array([6.7, 8.9, 11.8, 15.8, 21.1, 28.1, 37.5, 50, 66.7, 88.9, 118.6])
    smps_diam_list = np.array([10, 13, 18, 24, 32, 42, 56, 75, 100, 133, 178, 237])

    ground_smps_time = ground_smps_time - 21600
    date = '20140721'
    unix_epoch_of_21July2014_0001 = 1405900800
    if date == '20140721':
        trackpdtime = pd.to_datetime(np.asarray(ground_smps_time) + unix_epoch_of_21July2014_0001, unit='s')

    ax4 = fig.add_subplot(gs[3, :])
    plt.pcolormesh(trackpdtime, smps_diam_list, ground_smps.T, norm=colors.LogNorm(vmin=10, vmax=110000),
                   cmap='RdBu_r')
    plt.colorbar(label='dN/dlog$_{Dp}$\n(# cm$^{-3}$)', extend='max')
    plt.ylabel('Diameter\n (nm)')
    plt.ylim(10, 300)
...
def read_BAO21_precursor():
    ...
    BAO21_nh3_data = np.loadtxt(
        ...
        '-TOWER_20140721_R1.ict',
        delimiter=',', skiprows=48635, usecols=1, max_rows=55800)
    BAO21_nh3_data = ma.masked_where(BAO21_nh3_data < 0, BAO21_nh3_data)
    BAO21_nh3_time = np.loadtxt(
        '/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/FRAPPE-QCTILDAS-NH3_GROUND-BAO'
        '-TOWER_20140721_R1.ict',
        delimiter=',', skiprows=48635, usecols=0, max_rows=55800)
    ...
    unix_epoch_of_21July2014_0001 = 1405900800
    if date == '20140721':
        trackpdtime = pd.to_datetime(np.asarray(mask_nh3_time) + unix_epoch_of_21July2014_0001, unit='s')
    ...
    xmin = '2014-07-21 07:30:00'
    xmax = '2014-07-21 23:00:00'
    plt.xlim(xmin, xmax)  # corresponding to ground SMPS data,
    ...
    BAO21_so2_data = np.loadtxt(
        ...
        delimiter=',', skiprows=305, usecols=2, max_rows=930)  # 305=730AM lt
    BAO21_so2_data = ma.masked_where(BAO21_so2_data < 0, BAO21_so2_data)
    BAO21_so2_time = np.loadtxt(
        '/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/Frappe-SO2_GROUND-BAO-TOWER_20140721_R0.ict',
        delimiter=',', skiprows=305, usecols=0, max_rows=930)
    BAO21_so2_time = BAO21_so2_time - 21600  # to MT

    interp_gas = interp1d(BAO21_nh3_time, BAO21_nh3_data, kind='nearest', fill_value='extrapolate')
    so2_gas_track = interp_gas(np.asarray(BAO21_so2_time))  # from 60s to 1s
    ...
    plt.xlim(xmin, xmax)
    # plt.ylim(0.01, 20)
    print('mean:BAO21_nh3_data', np.nanmean(BAO21_nh3_data))
...
def groundmobile_windrose():  # line 561=730am LT,
    # 0721
    groundmobile_ws = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/DISCOVERAQ'
                                 '-LARGE_GROUND-MOBILE_20140721_R0.ict', delimiter=',', skiprows=561,
                                 usecols=9, max_rows=930)  # Seconds after midnight MDT
    groundmobile_wd = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/DISCOVERAQ'
                                 '-LARGE_GROUND-MOBILE_20140721_R0.ict', delimiter=',', skiprows=561,
                                 usecols=10, max_rows=930)  # Seconds after midnight MDT
    groundmobile_time = np.loadtxt('/jet/home/ding0928/box_model/data/overall_code/ground_ob_data/DISCOVERAQ'
                                   '-LARGE_GROUND-MOBILE_20140721_R0.ict', delimiter=',', skiprows=561,
                                   usecols=0, max_rows=930)  # Seconds after midnight MDT
    groundmobile_ws = ma.masked_where(groundmobile_ws < 0, groundmobile_ws)
    groundmobile_wd = ma.masked_where(groundmobile_wd < 0, groundmobile_wd)

    sl = [0, 0.8, 1.6, 2.6, 3.8, 4.6, 5.5, 6.5]
    ax = new_axes()
    ax.contourf(groundmobile_wd, groundmobile_ws, bins=sl, normed=True, cmap=cm.cool)
    set_legend(ax)
    print('mean:groundmobile_wd', np.nanmean(groundmobile_wd))
    print('groundmobile_wd[-1]', groundmobile_wd[-1])  # 246.58
    plt.show()
...
merge_obs_method()
aerosol_3_10nm()
read_groundmobile_smps()
read_BAO21_precursor()
groundmobile_windrose()
