...
from obser_for_Merge_p3b_altagl import merge_obs_method
    ...
    for itime in range(0,len(df.altitude)):
        ...
        if minutes[itime]==0 and seconds[itime]==0:
            ...
            if (date=='20160511' and hours[itime]%2==0) or date=='20160510' or date=='20160512':
                plt.text(df.longitude[itime],df.latitude[itime],timestring, fontsize=24)
...
def load_model(timeindexes,path,varlist,date,is_lam,is_dump):
    ...
    if 'SO4_Coa' in varlist: #co631
        so4_coa_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pb', iris.AttributeConstraint(STASH='m01s34i114'),is_lam)
        outputcubedict["SO4_Coa"] = so4_coa_mixing
    if 'BC_Coa' in varlist: #co631
        bc_coa_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pb', iris.AttributeConstraint(STASH='m01s34i115'),is_lam)
        outputcubedict["BC_Coa"] = bc_coa_mixing
    if 'OC_Coa' in varlist: #co631
        oc_coa_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pb', iris.AttributeConstraint(STASH='m01s34i116'),is_lam)
        outputcubedict["OC_Coa"] = oc_coa_mixing
    if 'SS_Coa' in varlist: # co631
        ss_coa_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pb', iris.AttributeConstraint(STASH='m01s34i117'),is_lam)
        outputcubedict["SS_Coa"] = ss_coa_mixing
    if 'NH4_Coa' in varlist: # co631
        nh4_coa_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pe', iris.AttributeConstraint(STASH='m01s34i135'),is_lam)
        outputcubedict["NH4_Coa"] = nh4_coa_mixing
    if 'NO3_Coa' in varlist: # co631
        no3_coa_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pe', iris.AttributeConstraint(STASH='m01s34i139'),is_lam)
        outputcubedict["NO3_Coa"] = no3_coa_mixing

    if 'SO4_Acc' in varlist: #co631
        so4_acc_mixing = load_um_cube(timeindexes,surfcoord,prefix+'_pb', iris.AttributeConstraint(STASH='m01s34i108'),is_lam)
        outputcubedict["SO4_Acc"] = so4_acc_mixing #kg/kg
    if 'BC_Acc' in varlist:  #co631
        bc_acc_mixing = load_um_cube(timeindexes, surfcoord, prefix +'_pb',iris.AttributeConstraint(STASH='m01s34i109'), is_lam)
        outputcubedict["BC_Acc"] = bc_acc_mixing  # kg/kg
    if 'OC_Acc' in varlist:  # co631
        oc_acc_mixing = load_um_cube(timeindexes, surfcoord, prefix + '_pb',iris.AttributeConstraint(STASH='m01s34i110'), is_lam)
        outputcubedict["OC_Acc"] = oc_acc_mixing  #kg/kg
    if 'SS_Acc' in varlist:  # co631
        ss_acc_mixing = load_um_cube(timeindexes, surfcoord, prefix+'_pb',iris.AttributeConstraint(STASH='m01s34i111'), is_lam)
        outputcubedict["SS_Acc"] = ss_acc_mixing  #kg/kg
    if 'NH4_Acc' in varlist: #co631
        nh4_acc_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pd', iris.AttributeConstraint(STASH='m01s34i134'),is_lam)
        outputcubedict["NH4_Acc"] = nh4_acc_mixing #kg/kg
    if 'NO3_Acc' in varlist: #co631
        no3_acc_mixing = load_um_cube(timeindexes,surfcoord,prefix + '_pe', iris.AttributeConstraint(STASH='m01s34i138'),is_lam)
        outputcubedict["NO3_Acc"] = no3_acc_mixing #kg/kg

    if 'SO4_Ait' in varlist: #co631
    ...
    if 'BC_Ait' in varlist: #,co631
        nolabel = '_pb'
        if is_lam:
            nolabel = '_pe'
        bc_ait_mixing = load_um_cube(timeindexes, surfcoord, prefix+nolabel,iris.AttributeConstraint(STASH='m01s34i105'), is_lam)
        outputcubedict["BC_Ait"] = bc_ait_mixing #kg/kg
    if 'OC_Ait' in varlist: #co631
        nolabel = '_pb'
        if is_lam:
            nolabel = '_pe'
        oc_ait_mixing = load_um_cube(timeindexes, surfcoord, prefix + nolabel,iris.AttributeConstraint(STASH='m01s34i106'), is_lam)
        outputcubedict["OC_Ait"] = oc_ait_mixing  # unit here is kg/kg
    if 'BC_Ait_ins' in varlist: #,co631
        bc_ait_mixing_ins = load_um_cube(timeindexes, surfcoord, prefix+'_pb',iris.AttributeConstraint(STASH='m01s34i120'), is_lam)
        outputcubedict["BC_Ait_ins"] = bc_ait_mixing_ins #kg/kg
    if 'OC_Ait_ins' in varlist: #co631
        nolabel = '_pd'
        if is_lam:
            nolabel = '_pb'
        oc_ait_mixing_ins = load_um_cube(timeindexes, surfcoord, prefix + nolabel,iris.AttributeConstraint(STASH='m01s34i121'), is_lam)
        outputcubedict["OC_Ait_ins"] = oc_ait_mixing_ins  # unit here is kg/kg
    if 'NH4_Ait' in varlist: #co631
    ...
    if 'N_All'in varlist or 'N_Nuc'in varlist or 'N_Ait_In' in varlist or 'N_Ait' in varlist or 'N_Acu' in varlist or 'N_Cor' in varlist \
        ...
        cor_num_mixing = load_um_cube(timeindexes, surfcoord, prefix+'_pb',iris.AttributeConstraint(STASH='m01s34i113'),is_lam)
...
def read_flight_data(date):
    ...
    #0731 is the same data structure with 0802
...
varlist = ['C5H8','C10H16','NO','NO2','O3','CO','CH4','NH3','P','T',
           ...
           'NO3_Ait','NH4_Ait','OC_Ait','BC_Ait','SO4_Ait',
           'NO3_Acc','NH4_Acc','OC_Acc','BC_Acc','SO4_Acc','SS_Acc',
           'NO3_Coa','NH4_Coa','OC_Acc','BC_Acc','SO4_Coa','SS_Coa',
           'BC_Ait_ins', 'OC_Ait_ins']
# varlist = []
# make_dataset=0
is_dump=0
if is_dump:
...
path_model='/jet/home/ding0928/cylc-run/u-cr293/share/cycle/20140729T0000Z/Regn1/resn_1/RA2M/um/'
path_glm = '/jet/home/ding0928/cylc-run/u-cr293/share/cycle/20140729T0000Z/glm/um/'
...
if make_dataset==1:
   ...
   # [df['model_SO2'],df['model_OH'],df['model_CO'],df['model_C5H8'],
    ...
    # print df.head()
    df.to_csv('interpolated_cr293p3b_dataset_'+date+'.csv')
else:
    df = pd.read_csv('interpolated_cr293p3b_dataset_'+date+'.csv',index_col=0, parse_dates=True)
    # print df.head()
    conc_dict = load_model(timeindexes_lam, path_model,varlist,date,1,0)
...
if 'NH3' in varlist:
    norm_array_nh3=[0.15,0.2,0.3,0.5,0.7,0.9,1.2,1.5,2,3,5,7,10]
    plot_comparison(df,df['NH3'],df['model_NH3'],df['glm_NH3'],(conc_dict['NH3'])[1,3,:,:],(conc_dict_glm['NH3'])[1,1,:,:],
                    'NH3 (ppbv)', 0.1,10,0.1,10,True,norm_array_nh3,'korus_nh3_'+date+'_bt116.png', date)
    plot_vertical(df,conc_dict['NH3'].coord('level_height'),df['NH3'],df['model_NH3'],df['glm_NH3'],10, 'NH3 (pptv)','korus_nh3_vp_'+date+'_bt116.png')
...
if 'NO' in varlist:
    norm_array_no = [5,10,20,50,100,200,300,500,700,1000,1500,2500,3500]
...
if 'SUMNC2' in varlist:
    ...
    plot_comparison(df,df['SUMNC2'],df['model_SUMNC2'],df['glm_SUMNC2'],(conc_dict['SUMNC2'])[1,3,:,:],(conc_dict_glm['SUMNC2'])[1,1,:,:],
                    'Cond sink (s-1)', 0.001,0.05,0.001,0.05,True,norm_array_sumnc2,'korus_sumnc2_'+date+'_bt116.png', date)

if 'N_Nuc' in varlist:
    norm_array_nuc=[100,300,500,800,1000,2000,3000,4000,5000,6000,7000,10000,20000,30000]
    plot_comparison(df,df['N_Nuc'],df['model_N_Nuc'],df['glm_N_Nuc'],(conc_dict['N_Nuc'])[1,3,:,:],(conc_dict_glm['N_Nuc'])[1,1,:,:],
                    'N$_{nuc(3-10nm)}$\n(#cm$^{-3}$)', 100,3e4,100,3e4,True,norm_array_nuc,'korus_nnuc_'+date+'_bt116.pdf', date)
    plot_vertical(df,conc_dict['N_Nuc'].coord('level_height'),df['N_Nuc'],df['model_N_Nuc'],df['glm_N_Nuc'],2e4, 'N_Nuc (#cm-3)','qual_fig.pdf')
...
if 'N_10' in varlist:
    ...
    plot_comparison(df,df['N_10'],df['model_N_10'],df['glm_N_10'],(conc_dict['N_10'])[1,3,:,:],(conc_dict_glm['N_10'])[1,1,:,:],
                    'N(>10nm) (# cm-3)', 500,2.5e4,500,2.5e4,True,norm_array_n10,'korus_n10'+date+'_bt116.png', date)
    plot_vertical(df,conc_dict['N_10'].coord('level_height'),df['N_10'],df['model_N_10'],df['glm_N_10'],2.5e4, 'N(10nm)','korus_N(10nm)_vp_'+date+'_bt116.png')
...
if 'N_All' in varlist:
    ...
    plot_comparison(df,df['N_All'],df['model_N_All'],df['glm_N_All'],(conc_dict['N_All'])[1,3,:,:],(conc_dict_glm['N_All'])[1,1,:,:],
                    'N$_{all(>3nm)}$\n(#cm$^{-3}$)', 500,3.5e4,500,3.5e4,True,norm_array_n10,'korus_nall'+date+'_bt116.png', date)
    plot_vertical(df,conc_dict['N_All'].coord('level_height'),df['N_All'],df['model_N_All'],df['glm_N_All'],3.5e4, 'N_All(3nm)','korus_Nall(3nm)_vp_'+date+'_bt116.png')
if 'N_100' in varlist:                                                                                                                                               
     # x=range(1,100)                                                                                                                                                
    ...
    plot_comparison(df,df['N_100'],df['model_N_100'],df['glm_N_100'],(conc_dict['N_100'])[1,3,:,:],(conc_dict_glm['N_100'])[1,1,:,:],                                    
                    'N >100nm (cm$^{-3}$ stp)', 10,2000,10,2000.0,True,norm_array_num,'korus_n100_'+date+'_bt116.png', date)                                               
    plot_vertical(df,conc_dict['N_100'].coord('level_height'),df['N_100'],df['model_N_100'],df['glm_N_100'],5000, 'N_100 (cm-3 stp)','korus_n100_vp_'+date+'_bt116.png')      
