...
def load_model(timeindexes,path,varlist,date,is_lam,is_dump):
    ...
    if 'N_All'in varlist or 'N_Nuc'in varlist or 'N_Ait_In' in varlist or 'N_Ait' in varlist or 'N_Acu' in varlist or 'N_Cor' in varlist \
        ...
        if is_lam:
            ...
            # nuc_num_mixing = load_um_cube(timeindexes, surfcoord, prefix + numlabel,
            #                               iris.AttributeConstraint(STASH='m01s34i101'), is_lam)
            ait_num_mixing = load_um_cube(timeindexes, surfcoord, prefix + numlabel,
                                          iris.AttributeConstraint(STASH='m01s34i103'), is_lam)
            acc_num_mixing = load_um_cube(timeindexes, surfcoord, prefix + numlabel,
                                          iris.AttributeConstraint(STASH='m01s34i107'), is_lam)
            cor_num_mixing = load_um_cube(timeindexes, surfcoord, prefix + numlabel,
                                          iris.AttributeConstraint(STASH='m01s34i113'), is_lam)
            aitin_num_mixing = load_um_cube(timeindexes, surfcoord, prefix + numlabel,
                                            iris.AttributeConstraint(STASH='m01s34i119'), is_lam)
            n10_num_mixing = ait_num_mixing + acc_num_mixing + cor_num_mixing + aitin_num_mixing  # >10nm number concentration
            nall_num_mixing = n10_num_mixing
...
varlist = ['C5H8','C10H16','NO','NO2','O3','CO','CH4','NH3','P','T',
            'N_Nuc','N_Ait','N_Acu','N_Cor','N_Ait_In','N_All',
            'D_Nuc','D_Ait','D_Acu','D_Cor','D_Ait_In',
           'NO3_Ait','NH4_Ait','SO4_Ait',
           'SO4_2','NO3','NH4','SUMNC2','N_10']
# varlist = []
# make_dataset=0
is_dump=0
if is_dump:
...
path_model='/ocean/projects/atm200005p/earhg/cylc-run/u-cp754/share/cycle/20140729T0000Z/Regn1/resn_1/RA2M/um/'
path_glm = '/ocean/projects/atm200005p/earhg/cylc-run/u-cp754/share/cycle/20140729T0000Z/glm/um/'
...
if 'N_Nuc' in varlist:
    norm_array_oh=[100,300,500,800,1000,2000,3000,4000,5000,6000,7000,10000]
    plot_comparison(df,df['N_Nuc'],df['model_N_Nuc'],df['glm_N_Nuc'],(conc_dict['N_Nuc'])[1,3,:,:],(conc_dict_glm['N_Nuc'])[1,1,:,:],
                    'N_Nuc(3-10nm) (#cm-3)', 100,1e4,100,1e4,True,norm_array_oh,'korus_nnuc_'+date+'_bt116.pdf', date)
    plot_vertical(df,conc_dict['N_Nuc'].coord('level_height'),df['N_Nuc'],df['model_N_Nuc'],df['glm_N_Nuc'],5e3, 'N_Nuc (#cm-3)','qual_fig.pdf')
...
if 'N_All' in varlist:
    ...
    plot_comparison(df,df['N_All'],df['model_N_All'],df['glm_N_All'],(conc_dict['N_All'])[1,3,:,:],(conc_dict_glm['N_All'])[1,1,:,:],
                    'N_all(>3nm) (# cm-3)', 500,3.5e4,500,3.5e4,True,norm_array_n10,'korus_nall'+date+'_bt116.png', date)
