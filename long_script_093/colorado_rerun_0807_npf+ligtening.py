...
from obser_for_Merge_0807_altagl import merge_obs_method
...
def read_flight_data(date):
    flight_path='/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-reveal_p3b_20140807_r0.ict'

    tracktime,alt,lat,lon = read_flight_track(flight_path,100, 4,2,3) # .checked 0807
    tracktime,rh,temp,pres2 = read_flight_track(flight_path, 100, 36, 18, 24)  # static pressure+static temp
    ...
    print('temptrack',temp)
    print('pres2track',pres2)
    ...
    c5h8_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/discoveraq-PTRTOF-NMHCs_P3B_20140807_R0.ict'
    c5h8time, c5h8,nh3,dummy = read_flight_track(c5h8_path, 67, 17,3,1) #.checked 0807
    c5h8track = interp_flight_data(tracktime, c5h8time, c5h8, -99) #ppbv
    nh3track = interp_flight_data(tracktime, c5h8time, nh3, -99) #ppbv

    no_o3_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-NOXYO3_P3B_20140807_R0.ict'
    notime, no, no2, o3 = read_flight_track(no_o3_path, 47, 3, 5, 6) #checked 0807
    ...
    co_p3b_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-DACOM_P3B_20140807_R0.ict '
    cotime, co, ch4, dummy = read_flight_track(co_p3b_path, 37, 1, 2,1)  # checked 0807
    cotrack = interp_flight_data(tracktime, cotime, co, -99)
    ch4track = interp_flight_data(tracktime, cotime, ch4, -99)

    n_nuc_p3b_path = '/jet/home/ding0928/Colorado/Colorado/p-3b/DISCOVERAQ-LARGE-CNC_P3B_20140807_R0.ict'
    n_nuctime, n3, n10, dummy = read_flight_track(n_nuc_p3b_path, 47, 1, 2,1)  # checked 0807
    ...
    d= {'time':pd.Series(trackpdtime), 'latitude':pd.Series(np.asarray(lat)),'longitude':pd.Series(np.asarray(lon)),
        ...
        'NH3':pd.Series(nh3track),'NO':pd.Series(notrack),'C5H8':pd.Series(c5h8track),
...
# def colorado_mod_J():
date= '20140807'
varlist = ['C5H8','NO','NO2','O3','CO','CH4','NH3','P','T',
...
if date=='20140807':
    timeindexes_glm=['012','015','018']  #0807+12h=0807_12pm
    timeindexes_lam=['012','018'] #0807+12h=0807_12pm
    lonmin=-112
...
if make_dataset==1:
    ...
    df.to_csv('interpolated_0807_lighenting_'+date+'.csv')
else:
    df = pd.read_csv('interpolated_0807_lighenting_'+date+'.csv',index_col=0, parse_dates=True)
    # print df.head()
    conc_dict = load_model(timeindexes_lam, path_model,varlist,date,1,0)
...
if 'SO2' in varlist:
    ...
    plot_vertical(df,conc_dict['SO2'].coord('level_height'),df['SO2'],df['model_SO2'],df['glm_SO2'],2500, 'SO$_{2}$ (pptv)','korus_so2_vp_'+date+'ion_npf_cs093.png')
